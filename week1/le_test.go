...
func TestLe15(t *testing.T) {
	...
	for _, u := range units {
		expected := IntsArray(u.expect).Sort()
		actual := IntsArray(threeSum(u.nums)).Sort()
		assert.ElementsMatch(t, expected, actual)
		actual = IntsArray(nSum(u.nums, 3, 0)).Sort()
		assert.ElementsMatch(t, expected, actual)
	}
}
...
func TestLe20(t *testing.T) {
	...
	for _, u := range units {
		...
		assert.Equal(t, u.expect, isValidII(u.s), fmt.Sprintf("%s expected: %v", u.s, u.expect))
	}
}
